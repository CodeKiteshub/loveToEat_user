...
class ProductService implements ProductServiceInterface {
  ...
  @override
  List<List<int?>> initializeSelectedVariationQty(List<Variation>? variations) {
    List<List<int?>> selectedVariationsQtyList = [];
    if(variations != null){
      for(int index=0; index<variations.length; index++){
      selectedVariationsQtyList.add([]);
        for(int i=0; i < variations[index].variationValues!.length; i++) {
          selectedVariationsQtyList[index].add(1);
        }
      }
    }
    return selectedVariationsQtyList;
  }
  ...
  @override
  int setVariationQuantity(int addOnQty, bool isIncrement, String? stockType, int? currentStock, Variation variation, int selectedVariationQty) {
    int qty = addOnQty;
      if (isIncrement) {

if (variation.max! > selectedVariationQty) {
        if(stockType != 'unlimited' && currentStock != null && qty >= currentStock) {
        showCustomSnackBar('${'maximum_addon_limit'.tr} $currentStock', showToaster: true);
      } else {
        qty = qty + 1;
      }
}
  ...
  @override
  List<List<bool?>> setCartVariationIndex(int index, int i, List<Variation>? variations, bool isMultiSelect, List<List<bool?>> selectedVariations, List<List<int?>> selectedVariationsQtyList) {
    ...
    if(!isMultiSelect) {
      ...
      if(!resultVariations[index][i]! && selectedVariationLength(resultVariations,selectedVariationsQtyList, index) >= variations![index].max!) {
  ...
  @override
  int selectedVariationLength(List<List<bool?>> selectedVariations, List<List<int?>> selectedVariationQty, int index) {
    int length = 0;
int k = 0;
    ...
    for(bool? isSelected in selectedVariations[index]) {
      ...
      if(isSelected!) {
      for (var i = 0; i < selectedVariationQty[index][k]!; i++) {
          length++;
