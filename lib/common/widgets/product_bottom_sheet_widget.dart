                              ...
                              itemBuilder: (context, index) {
                                ...
                                if(product!.variations![index].required!){
                                  ...
                                  for (var value in productController.selectedVariations[index]) {
                                    if(value == true){
                                      for (var i = 0; i < productController.selectedVariationsQtyList[index][k]!; i++) {
                                        selectedCount++;
                                      }
                                    }
                                    k++;
                                          ...
                                          return Padding(
                                            ...
                                            child: InkWell(
                                              ...
                                              child: Row(children: [
                                                  ...
                                                  child: Row(crossAxisAlignment: CrossAxisAlignment.center, children: [
                                                    product!.variations![index].multiSelect! ? Checkbox(
                                                      ...
                                                      onChanged:(bool? newValue) {
                                                        if (product!.variations![index].variationValues![i].isMultiple!) {
                                                          if (productController.selectedVariations[index][i]!) {
                                                            if (productController.selectedVariationsQtyList[index][i]! == 1) {
                                                              
                                                        productController.setCartVariationIndex(index, i, product, product!.variations![index].multiSelect!);
                                                        productController.setExistInCartForBottomSheet(product!, productController.selectedVariations);
                                                            }
                                                          } else {
                                                            if (product!.variations![index].max! > selectedCount) {
                                                               productController.setCartVariationIndex(index, i, product, product!.variations![index].multiSelect!);
                                                        productController.setExistInCartForBottomSheet(product!, productController.selectedVariations);
                                               ...
                                               productController.selectedVariations[index][i] == false ?
                                               const SizedBox()
                                               :  product!.variations![index].variationValues![i].isMultiple! ? Container(
                                          height: 25, width: 90,
                                          ...
                                          child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                                            Expanded(
                                              child: InkWell(
                                                onTap: () {
                                                  if (productController.selectedVariationsQtyList[index][i]! > 1) {
                                                    productController.setVariationQuantity(false, index, i, product!.variations![index].variationValues![i].stockType, product!.variations![index].variationValues![i].currentStock,product!.variations![index],selectedCount);
                                                  } else {
                                                ...
                                                child: Center(child: Icon(
                                                  (productController.selectedVariationsQtyList[index][i]! > 1) ? Icons.remove : CupertinoIcons.delete, size: 18,
                                                  color: (productController.addOnQtyList[index]! > 1) ? Theme.of(context).primaryColor : Theme.of(context).colorScheme.error,
                                                )),
                                              ),
                                            ),
                                            Text(
                                              productController.selectedVariationsQtyList[index][i].toString(),
                                              style: robotoMedium.copyWith(fontSize: Dimensions.fontSizeDefault),
                                            ),
                                            Expanded(
                                              child: InkWell(
                                                onTap: () => productController.setVariationQuantity(true, index, i, product!.variations![index].variationValues![i].stockType, product!.variations![index].variationValues![i].currentStock, product!.variations![index],selectedCount),
    ...
    if(product!.variations != null && product!.variations!.isNotEmpty){
      for(int index=0; index<product!.variations!.length; index++) {
        if(!product!.variations![index].multiSelect! && product!.variations![index].required!
         ...
         int n = productController.selectedVariationLength(productController.selectedVariations, index);
         log(n.toString());
      ...
      for(int index = 0; index< product.variations!.length; index++) {
        for(int i=0; i<product.variations![index].variationValues!.length; i++) {
          if(productController.selectedVariations[index].isNotEmpty && productController.selectedVariations[index][i]!) {
            variationPrice += PriceConverter.convertWithDiscount(product.variations![index].variationValues![i].optionPrice! * productController.selectedVariationsQtyList[index][i]!, discount, discountType)!;
          }
        }
      }
    }
  ...
  double _getVariationPrice(Product product, ProductController productController) {
    ...
    if(product.variations != null){
      for(int index = 0; index< product.variations!.length; index++) {
        for(int i=0; i<product.variations![index].variationValues!.length; i++) {
          if(productController.selectedVariations[index].isNotEmpty && productController.selectedVariations[index][i]!) {
            variationPrice += PriceConverter.convertWithDiscount(product.variations![index].variationValues![i].optionPrice! * productController.selectedVariationsQtyList[index][i]!, 0, 'none')!;
          }
        }
      }
    }
